  ...
  switch (modelId) {
    ...
    default:
      data = {
        ...
        model: "gpt-3.5-turbo-instruct",
        prompt: "Responde claramente: " + text,
        temperature: 0.5,
        max_tokens: 60,
        top_p: 0.3,
  ...
  request.onreadystatechange = function () {    
    if (this.readyState === 4) {
      ...
      if (
        ...
        addMessage(
          "Assistant",
          "Lo siento, no puedo proporcionar una respuesta en este momento..",
          true
        );
      }
...
clearButton.addEventListener("click", () => {
  if (
    confirm(
      "¿Esta seguro de que desea eliminar la conversación? Esta accion no se puede deshacer."
    )
  ) {
    const transaction = db.transaction(["messages"], "readwrite");
    const objectStore = transaction.objectStore("messages");
...
console.info("Total usado localStorage : " + totalSize + " bytes");
